...
class HelpView : View(Utils.APP_TITLE + ": Ajuda") {
    override val root = borderpane {
        ...
        center = vbox {
            ...
            add(label("(Database) -> Cerca estada (cerca per codi d'estada, NIF. Cerca per nom. Cerca estades en curs) | Seguiment | Nova | Obre PDF | Estat (Documentada, Finalitzada, Renùncia) | Tanca"))
            add(label("(Edit) -> Admesos (del curs actual) | SSTT (Tots els SSTT's) | Visites (del curs actual)"))
            ...
            add(label("(Formació) -> Autoritzacions -> Despeses (Autorització d'una ordre de serveis) | FORTECO (Formació Tècnica Coordinada)"))
            ...
            add(label("(1)  Cerca estada per codi d'estada o NIF"))
