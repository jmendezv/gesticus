...
const val allSeguimentEmpresesByIdEmpresa =
    ...
    * */
    ...
    private fun creaDirectori(): String {
        ...
        val newDir = "${PATH_TO_DESPESES}forteco_${transaction}"
        File(newDir).mkdir()
        return newDir
    }
...
//                .replace("?2", "${model.motiuDesplaçament.value}"),
    ...
    private fun getAllForteco() {
        val allEmpresesByMunicipiStatement = conn.prepareStatement(findSollicitantsByNif)
        allEmpresesByMunicipiStatement.setString(1, currentCourseYear())
        val result = allEmpresesByMunicipiStatement.executeQuery()
        if (result.next()) {
            with(result) {
                val nom = getString("professors_nom")
...
//                sollicitant = Sollicitant(nom, email, cos, centre, unitat)
    ...
    * */
    fun doForteco() {
        ...
        getAllForteco()
        creaPdfs(dir)
        creaZip(dir, "${PATH_TO_DESPESES}${dir}.zip")
        lliuraZip("", "")
    }
